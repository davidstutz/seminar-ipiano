...
\begin{definitionmd}
	Let $g : \dom(g) \subset \mathbb{R}^n \rightarrow \mathbb{R} \cup \{\infty\}$ be a proper convex and lower semi continuous function. Then, the subdifferential $\partial g(x)$ of $g$ at position $x \in \dom(g)$ is defined as
	\begin{align}
...
\begin{align}
	\hat{\partial} g(x) := \{w \in \mathbb{R}^n | \lim \inf_{x^{(n)} \rightarrow x, x^{(n)} \neq x} \frac{g(x^{(n)}) - g(x) - w^T(x^{(n)} - x)}{|x^{(n)} - x|} \geq 0\},
\end{align}
...
\begin{align}
	...
	\end{aligned}
\end{align}
However, following \cite[Prop. 8.12]{Rockafellar:1970}, the equivalence of Equations \eqref{eq:limiting-subdiff} and \eqref{eq:limiting-subdiff} follows if $g$ is proper convex.
...
\begin{lemmamd}
	For $g: \dom(g) \subset \mathbb{R}^n \rightarrow \mathbb{R} \cup \{\infty\}$ and $x \in \mathbb{R}^n$ with $g(x) < \infty$, the set $\partial g(x)$ is closed.\label{lemma:closure}
\end{lemmamd}
\vskip -1em
\begin{lemmamd}
	Let $h = f + g$ with $f: \mathbb{R}^n \rightarrow \mathbb{R}$ in $C^1$ and $g:\dom(g) \subset \mathbb{R}^n \rightarrow \mathbb{R} \cup \{\infty\}$ convex. Then, for $x \in \dom(h)$ it holds
	\begin{align}
...
\subsubsection{miPiano -- Algorithm \ref{alg:nmipiano}}
...
\end{align}
which follows from Lemma \ref{lemma:lipschitz}. Usually, $x^{(n + 1)}$ is re-estimated until a
\begin{align}
...
\begin{proof}
	By the definition of $\delta_n$ and $\gamma_n$ in Algorithm \ref{alg:ipiano} it directly follows that $0 < \gamma_n \leq \delta_n$. Now, rearranging
	\begin{align}
	...
	\end{align}
	the upper bound for $\alpha_n$ follows and for $\beta_n < 1$ and $c_1$ small enough the existence of an appropriate $\alpha_n$ follows.
\end{proof}
	...
	\begin{algorithmic}[1]
		...
		\FOR{$n = 1, \ldots$}
			...
			\REPEAT
				\STATE{$L_n := \eta L_n$ // Next estimate of $L_n$.}
				\STATE{choose $\alpha_n < \frac{2(1 - \beta)}{L_n}$}
				\STATE{$\tilde{x}^{(n + 1)} = {\prox {\alpha g}} \left(x^{(n)} - \alpha \nabla f(x^{(n)}) + \beta (x^{(n)} - x^{(n - 1)})\right)$ // Re-estimate $\tilde{x}^{(n + 1)}$.}
			\UNTIL{\eqref{eq:lipschitz-condition} is satisifed for $\tilde{x}^{(n + 1)}$}
			\STATE{//Estimated $L_n$ satisfies Equation \eqref{eq:lipschitz-condition} and $\tilde{x}^{(n + 1)}$ is next iterate:}
			\STATE{$x^{(n + 1)} := \tilde{x}^{(n + 1)}$}
		\ENDFOR
...
\begin{lemmamd}
	$H_{\delta_n}$ satisfies \ref{cond:2}, \ie for each $n \in \mathbb{N}$ and for a fixed $b > 0$, there exists $w^{(n + 1)} \in \partial H(z^{(n + 1)})$ with
	\begin{align}
		\|w^{(n + 1)}\|_2 \leq \frac{b}{2} (\Delta_n + \Delta_{n + 1}).
	\end{align}
\end{lemmamd}
...
\begin{definitionmd}
	A function $H : \mathbb{R}^n \times \mathbb{R}^n \rightarrow \mathbb{R} \cup \{\infty\}$ has the Kurdyka-Lojasiewicz property, if for each $z^\ast \in \dom(\partial H)$ there exist $\eta \in (0,\infty]$, a neighborhood $U$ of $z^\ast$, and a continuous concave function $\phi : [0, \eta) \rightarrow \mathbb{R}_{+}$ such that
	\begin{itemize}
		\item[--] $\phi(0) = 0$,
		\item[--] $\phi \in C^1((0, \eta))$,
		\item[--] for all $s \in (0, \eta)$, $\phi'(s) > 0$,
...
Obviously, this recursion can be broken when expecting $\beta_0$ to be given as parameter (as it is more difficult to make a good guess for $\delta_{-1}$ than $\beta_0$). Then, it remains to find $L_0$ in order to deduce $\alpha_0$ as in Equation~\eqref{eq:alpha-0}. Algorithm \ref{alg:ipiano} depicts a simple backtracking scheme to estimate $L_0$ given a user provided estimate $L_{-1}$. Instead, we propose to estimate $L_0$ directly around $x^{(0)}$:
\begin{align}
	&\|\nabla f(x^{(0)}) - \nabla f(\hat{x}^{(0)})\|_2 \leq L_0 \|x^{(0)} - \hat{x}^{(0)}\|_2\\
	\Leftrightarrow\quad& \frac{\|\nabla f(x^{(0)}) - \nabla f(\hat{x}^{(0)})\|_2}{\|x^{(0)} - \hat{x}^{(0)}\|_2} \leq L_0\label{eq:estimate-L}
\end{align}
for an appropriate $\hat{x}^{(0)}$ which can be chosen according to
\begin{align}
	\hat{x}^{(0)} = x^{(0)} - \nabla f(x^{(0)}).
\end{align}
Given $\beta_0$ and $L_0$, $\alpha_0$ can be determined using the inequality in Equation \eqref{eq:alpha-0} subject to the requirement
...
The remaining non-determinism can be resolved by considering a fixed number of discrete steps
\begin{align}
	\alpha_0^{(k)} := a_0 - k*\frac{a_0 - c_1}{K}\quad\text{ with }\quad a_0 := \frac{2(1 - \beta_0)}{L_0},\text{ }K \gg 100\quad\text{ and }\quad k = 1,\ldots,K.
\end{align}
until an $\alpha_0^{(k)}$ is found satisfying the requirements.
...
We need to resolve the nondeterminism in Algorithm \ref{alg:ipiano}, Lines \ref{line:ipiano-alpha} and \ref{line:ipiano-beta}. Given the current estimate of $L_n$, see Line \ref{line:ipiano-L}, we follow a similar strategy as used for initialization. In particular, we evaluate the requirement
\begin{align}
	\delta_n \geq \gamma_n \geq c_2
\end{align}
for discrete steps
\begin{align}
	\beta_n^{(j)} &:= b_n - j*\frac{b_n}{J}\quad\text{ with }\quad b_n := \frac{\delta_{n - 1} + \frac{L_n}{2}}{c_2 + \frac{L_n}{2}},\text{ }J \gg 100\quad\text{ and }\quad j = 0,\ldots,J,\\
	\alpha_n^{(k)} &:= a_n - k*\frac{a_n - c_1}{K}\quad\text{ with }\quad a_n := \frac{2(1 - \beta_n)}{(L_n + 2 c_2)},\text{ }K \gg 100\quad\text{ and }\quad k = 1,\ldots,K
\end{align}
and stop as soon as the requirements are satisfied. As alternative to Line \ref{line:ipiano-backtracking}, \ie using $L_{n - 1}$ as starting point for backtracking, we re-estimate $L_n$ using Equation \eqref{eq:estimate-L} (adapted for $n \geq 0$). The disadvantage of using $L_{n - 1}$ is that backtracking guarantees $L_{n} \geq L_{n - 1}$ which is problematic as high $L_n$ induces low $\alpha_n$ for fixed $\beta_n$ slowing down convergence. Re-estimating $L_n$, in contrast, also allows $L_n < L_{n - 1}$.
...
\begin{figure}
	...
	\begin{subfigure}[t]{0.16\textwidth}
		\includegraphics[scale=0.15]{pictures/segmentation/absolute/{3096_0.200000_seg}.png}
	\end{subfigure}
	\caption{Segmentation results for an image from the Berkeley Segmentation Dataset \cite{ArbelaezMaireFowlkesMalik:2011} for thresholds $\tau = -0.1, -0.2, 0.0, 0.1, 0.2$ and using $g_{\text{abs}}$; the foreground segment $S_f$ is depicted in white.}
	\label{fig:segmentation-absolute}
\end{figure}
